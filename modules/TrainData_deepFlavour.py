...
class TrainData_image_plusDeltaR(TrainData_fullTruth):
    '''
    This class is for simple jetimiging
    '''
    ...
    def __init__(self):
        ...
        Constructor
        '''
        super(TrainData_image_plusDeltaR,self).__init__()

        self.addBranches(['jet_pt', 'jet_eta','nCpfcand','nNpfcand','nsv','npv'])
        self.registerBranches(['Cpfcan_ptrel','Cpfcan_eta','Cpfcan_phi','Cpfcan_deltaR',
                               'Npfcan_ptrel','Npfcan_eta','Npfcan_phi','Npfcan_deltaR',
                               'nCpfcand','nNpfcand',
                               'jet_eta','jet_phi','jet_pt'])
    ...
    def readFromRootFile(self,filename,TupleMeanStd, weighter):
        from preprocessing import MeanNormApply, MeanNormZeroPad, createDensity, createDensityMap, createCountMap, MeanNormZeroPadParticles
        import numpy
        from stopwatch import stopwatch
...
#                                   'nNpfcand',-1)
    ...
    @staticmethod
    def regression_model(input_shapes):
        inputs, x = TrainData_image_plusDeltaR.base_model(input_shapes)
        predictions = Dense(2, activation='linear',init='normal')(x)
        return Model(inputs=inputs, outputs=predictions)
...
class TrainData_image_validPadding(TrainData_fullTruth):
    '''
    This class is for simple jetimiging
    '''
    ...
    def __init__(self):
        ...
        Constructor
        '''
        super(TrainData_image_validPadding,self).__init__()
    ...
    def base_model(input_shapes):
        ...
        x =   Convolution2D(64, (8,8)  , border_mode='valid', activation='relu',kernel_initializer='lecun_uniform')(x_map)
        x = MaxPooling2D(pool_size=(2, 2))(x)
        x =   Convolution2D(64, (4,4) , border_mode='valid', activation='relu',kernel_initializer='lecun_uniform')(x)
        x = MaxPooling2D(pool_size=(2, 2))(x)
        x =   Convolution2D(64, (4,4)  , border_mode='valid', activation='relu',kernel_initializer='lecun_uniform')(x)
        x = MaxPooling2D(pool_size=(2, 2))(x)
        x = Flatten()(x)
    ...
    @staticmethod
    def regression_model(input_shapes):
        inputs, x = TrainData_image_validPadding.base_model(input_shapes)
        predictions = Dense(2, activation='linear',init='normal')(x)
        return Model(inputs=inputs, outputs=predictions)
